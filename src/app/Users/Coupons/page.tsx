...
import { Ticket, Gift, AlertCircle, Copy, Check, CalendarClock } from "lucide-react";
import { getUserCoupons, collectCoupon, type Coupon } from "@/app/actions/coupon";
...
export default function UserCouponsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (!liff.isLoggedIn()) {
            // Attempt to get from session if dev mode or already authed
            const sessionUser = sessionStorage.getItem("user_id");
            if (sessionUser) {
                setUserId(sessionUser);
                loadCoupons(sessionUser);
            } else {
        ...
        // Here we assume the backend/session mapping logic is handled or we use the profile.userId to find our internal ID
        // For consistency with Membership page which uses sessionStorage 'user_id'
        const sessionUser = sessionStorage.getItem("user_id");
        if (sessionUser) {
           setUserId(sessionUser);
           loadCoupons(sessionUser);
        } else {
           // Fallback or redirect to register/login flow if needed
  ...
  const loadCoupons = async (uid: string) => {
      try {
          const data = await getUserCoupons(uid);
          setCoupons(data);
      } catch (error) {
                ...
                <div className="space-y-3">
                    {coupons.map((coupon) => (
                        <div 
                            ...
                            className={`relative w-full h-32 flex drop-shadow-sm transition-transform active:scale-[0.99] group ${
                                coupon.is_expired ? 'opacity-60 grayscale' : 'hover:-translate-y-1'
                            }`}
                        >
                            ...
                            <div 
                                ...
                                style={{
                                    backgroundImage: coupon.is_expired 
